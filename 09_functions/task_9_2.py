...
def generate_access_config(intf_vlan_mapping, trunk_template):
    ...
    for port in intf_vlan_mapping:
        access_cfg.append('interface '+port)
        for command in trunk_template:
            access_cfg.append(f"{command} {', '.join(str(x) for x in intf_vlan_mapping[port])}") if command.find("allowed vlan")>=1 else access_cfg.append(f"{command}")
    return access_cfg
