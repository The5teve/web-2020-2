...
def parse_cdp_neighbors(command_output):
    ...
    unit=''
    my_dict=dict()
    for lett in command_output:
        if lett!='\n':
            mystr+=lett
        else:
            if mystr.find('show cdp neighbors')>=0:
                unit=mystr.split('>')[0]
            elif mystr.find('/')>=0:

                my_dict[unit,mystr.split()[1]+mystr.split()[2]]=mystr.split()[0],mystr.split()[-2]+mystr.split()[-1]
            mystr=''            
    return my_dict
    """
...
if __name__ == "__main__":
    with open("sh_cdp_n_sw1.txt") as f, open("sh_cdp_n_r3.txt") as f1:
        print(parse_cdp_neighbors(f.read()))
        print(parse_cdp_neighbors(f1.read()))
